...
2. ~~Root-cause the negative-CO2-enhancement plants~~ — **Mostly done** (§12.6): Koradi and Tamnar's negative values are statistically consistent with zero (no detectable signal, not a real negative anomaly), resolving the apparent Tamnar wind-alignment contradiction. **ShriSingajiMalwa's negative enhancement is statistically significant and remains genuinely unexplained** — *(still open, narrowed to one facility)*.
...
**12.6 Root-caused the negative-CO2-enhancement anomaly (§11 item 2).** Wrote `diagnose_negative_enhancement.py`, reusing `diagnose_talcher.py`'s near/background and background-definition-sensitivity functions plus a new statistical-significance check (is the near-minus-background difference distinguishable from zero given its standard error?). **Result: 2 of 3 facilities (Koradi z=−1.58, Tamnar z=−1.33) are statistically consistent with zero** — the honest characterization is "no detectable enhancement given available signal-to-noise," not a real negative signal, which also resolves the apparent Tamnar wind-alignment contradiction (wind alignment and CO2 signal-to-noise are computed independently; a facility can have plausible NO2 plume geometry and simultaneously too weak a CO2 signal to measure). **ShriSingajiMalwa is different: its negative enhancement (−1.052 ppm, z=−5.32) is statistically significant**, a genuine unresolved anomaly, not noise — this one item remains open. All three show elevated background-definition sensitivity vs. the well-bracketed Rihand baseline (4% IME-proxy range), consistent with the same thin-signal fragility pattern the Talcher diagnosis identified.

**12.7 What remains open after this session.** The A1→A5 Q-correcting ablation ladder (`RESEARCH_PLAN.md` §9) is still not built — it needs an independent ground-truth emissions source, since Climate TRACE is explicitly barred from use as a training label (§7). Track A's exhaustive LOFO recall (47.2%) reframes "improve facility-level generalization" (§11 item 6) as the primary open weakness, now with a concrete lead (facilities with ambiguous NO2+SO2+VIIRS signal generalize worst). ShriSingajiMalwa's statistically significant negative enhancement (§12.6) remains genuinely unexplained. §11 item 1 (facility-set completion) is otherwise unaffected by this session's work.
