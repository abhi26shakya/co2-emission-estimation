...
2. Facility-set expansion (→ Week 9, continued through the Aug 13 session)
3. Activity-signal extraction (→ committed same day as Week 9 completion)
4. Correction/fusion model combining Track A + Track B (→ attempted narrowly via `reliability_model.py`, Week 10; full model **not yet done**, see §11)
5. Leave-one-facility-out (LOFO) cross-validation (→ **not yet done**, see §11)
6. Climate TRACE benchmark (→ done, Week 10)
7. Full evaluation figures (→ **not yet done**, see §11)
...
1. **Complete the facility-set expansion** — commit the remaining uncommitted plant-processing work (Kahalgaon, Mouda, Chhabra, Farakka, Simhadri) and reconcile `NEXT_STEPS.md`'s claimed "20/20" status against the actual current entry count in `data/plant_results.json` (see §9.2).
2. **Root-cause the negative-CO2-enhancement plants** (ShriSingajiMalwa, Koradi, Tamnar) with a targeted diagnosis analogous to `diagnose_talcher.py`, since the current working hypothesis ("good wind/CO2 alignment implies trustworthy signal," from the Talcher diagnosis) is directly contradicted by Tamnar's case (best-ever wind alignment, still negative enhancement).
3. **Correction / fusion model combining Track A and Track B** (`RESEARCH_PLAN.md` §14 step 4) — the current activity-signal work is a first exploratory step (and an inconclusive one at N=7), not a finished fusion model.
4. **Leave-one-facility-out (LOFO) cross-validation** (`RESEARCH_PLAN.md` §14 step 5) — not yet implemented for either track.
5. **Full evaluation figures** (`RESEARCH_PLAN.md` §14 step 7) — not yet produced; existing figures are per-analysis-script outputs (Grad-CAM, false-alarm panels, per-plant CO2 dashboards), not a consolidated evaluation figure set.
6. **Improve Track A's facility-level generalization** — the current 67.3% / 8%-recall result (Week 11) is a diagnosis of the leakage problem, not yet a fix for the underlying generalization gap; more/varied training facilities, architecture changes, or a different training curriculum are open directions not yet attempted (Not documented / needs verification whether any specific next approach has been decided).
7. **Structural checkpoint-file locking**, beyond the current results-file-level `fcntl.flock` fix — per-granule checkpoint writes are still not lock-protected, which is what allowed the two 2026-08-13 duplicate-process race conditions to occur in the first place (caught operationally both times, not structurally prevented).
8. **Explain the 2026-07-10 → 2026-08-10 gap**, or otherwise confirm there is no missing/undocumented work from that period, for the record's completeness.
9. **Formalize a dependency manifest** (`requirements.txt` or equivalent) and, if desired, a basic test suite — currently absent.
...
*End of master research log. This document was compiled by reading the full git commit history (`git log --all`), all commit diffs for major milestones, every Markdown/text documentation file in the repository (`README.md`, `RESEARCH_PLAN.md`, `NEXT_STEPS.md`, `WEEK2_LOG.txt`–`WEEK11_LOG.txt`), all 39 Python source files, and the schemas/example contents of the JSON and CSV data files, as of 2026-08-13. Sections or claims marked "Not documented / needs verification" indicate information that was not found in the repository during this compilation and should not be treated as fact without further checking.*
