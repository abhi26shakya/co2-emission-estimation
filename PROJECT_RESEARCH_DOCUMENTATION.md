...
- **No formal test suite, `pyproject.toml`/environment file, `CLAUDE.md`, or `docs/` folder exists anywhere in this repository** — `requirements.txt` was added §12.12c, pinned to `/opt/miniconda3/envs/co2`. That pass also found scripts have inconsistently run under a second Python (the macOS framework build) missing `earthaccess`/`xarray`/`geemap`, undetected until now because this session's scripts happened not to need those specific packages. A test suite remains absent. This is stated here as a factual gap, not a criticism.
