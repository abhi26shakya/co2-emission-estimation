...
6. ~~Improve Track A's facility-level generalization~~ — **Substantially improved** (§12.9): exhaustive LOFO recall rose from 47.2% to **69.1%** by matching all facilities to 24-tile temporal depth (data augmentation tried separately and found not to help). Narrowed from a broad ~50%-of-facilities problem to two specific remaining 0%-recall cases, **Kudgi and ShriSingajiMalwa**. *(Narrowly open — two facilities, not a general weakness.)*
