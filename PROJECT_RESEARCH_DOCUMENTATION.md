...
- **Varon et al. 2018** — source of the Integrated Mass Enhancement (IME) method and the effective-wind-speed scaling factor α = 0.5 used in `physics_ime.py` (originally developed for CH4 plumes).
...
4. Correction/fusion model combining Track A + Track B (→ the uncertainty-*prediction* feasibility check flipped from negative (N=7) to positive (N=17, §12.3); a real Q-correcting model against genuine ground truth (CEA data) is now **done**, see §12.15)
...
**Pre-Week-2 (2026-07-01, commits `5ac9bc1`, `acda84b`):** Initial commit adds mostly-empty stub files, including `physics_ime.py` (which stays empty until Week 6) and `first_no2.py`, which is then fleshed out (+21 lines) as the earliest NO2 data-exploration script.
...
**Week 8 (2026-08-12, `2d807ea`):** Two structural improvements to `physics_ime.py`, per `RESEARCH_PLAN.md` §14's top-priority recommendation:
...
**Same day (`a0d70d0`):** The background-definition sensitivity discovered while diagnosing Talcher is folded directly into `physics_ime.py` as a formal **third uncertainty term** (`_bg_definition_rel_std()`), computed for all facilities alongside the existing wind and IME-bootstrap-sampling terms, and combined in quadrature into the final `q_rel_std`.
...
- **Track B:** all 20/20 candidate plants processed. 17/20 produce a `physics_ime.py` estimate (Mundra, Sipat, Simhadri excluded for a genuine 0-near-sounding coverage gap, not left unverified). The three previously-flagged negative-CO2-enhancement facilities (ShriSingajiMalwa, Koradi, Tamnar) are now fully root-caused (§12.6–§12.7): Koradi and Tamnar are statistically consistent with zero signal; ShriSingajiMalwa was a seasonal near/background sampling artifact whose sign flips positive under a same-month comparison. None represent a pipeline bug or genuine negative signal.
- Climate TRACE uncertainty-interval calibration: 71% (5/7, Week 10) → 53% (9/17, §12.5, larger sample) → **35.3% (6/17, §12.12a)** after month-stratifying `physics_ime.py`'s near/background comparison — each drop is a more honest number from a larger sample or a real bug fix, not a regression in methodology, but the trend means current calibration is meaningfully worse than early small-sample figures suggested.
...
1. ~~Complete the facility-set expansion~~ — **Done**: all 20/20 facilities processed; 17/20 produce a `physics_ime.py` estimate (the remaining 3 excluded for a genuine 0-near-sounding coverage gap, not left unverified). See §12.3.
2. ~~Root-cause the negative-CO2-enhancement plants~~ — **Done** (§12.6–§12.7): Koradi and Tamnar's negative values are statistically consistent with zero (no detectable signal, not a real anomaly); ShriSingajiMalwa's was a seasonal-sampling artifact (near-plant data all from January, background blended in April/May's ~4 ppm-higher seasonal baseline) — same-month comparison flips the sign to a small positive difference. All three resolved; none a real negative signal or pipeline bug. New follow-up item: month-stratify `physics_ime.py`'s near/background comparison generally.
3. ~~Correction / fusion model combining Track A and Track B~~ — **Done** (§12.15): found a real, independent, non-Climate-TRACE ground-truth source (CEA's CO2 Baseline Database, bottom-up from reported fuel consumption, not satellite-inferred) covering all 30 candidate facilities, and built the first genuine Q-correcting model against it. Baseline Track B accuracy vs. real ground truth: MAE 1.01 in log-ratio space, 47.1% bracketing (N=17). A single-feature LOO-CV correction using `bg_std_ppm` (background XCO2 noise) reduced MAE to 0.902, a genuine ~11% improvement. Still indicative (N=17, single-feature, one fiscal year), not production-validated, but this is the actual milestone this item called for.
...
**12.3 Reliability model re-run at N=17 (roadmap step 4).** `physics_ime.py` had only been run against 13 of 20 facilities; re-ran it against the full set, adding 4 more estimates (Kahalgaon, Mouda, Chhabra, Farakka — soundings existed but hadn't gone through the physics step). 17/20 facilities now have an estimate; only Mundra, Sipat, Simhadri remain excluded (genuine 0-near-sounding coverage gap). Re-ran `reliability_model.py` (§7.4's N=7 negative result) at N=17: **now a positive result** — `hit_days` predicts `q_rel_std` at r=−0.617, LOO R²=0.212 (MAE=0.095), versus no surviving correlation at N=7.
...
**12.12 Three smaller hardening items (§11 items 7–9).** (a) **Month-stratified `physics_ime.py`** (§12.7's flagged general gap): the background population (main annulus and each of the 5 alternate sensitivity-term definitions) is now restricted to only the months the near-plant zone was actually sampled in, when per-sounding dates exist — falling back to the unrestricted background when day data is missing or stratifying would leave too few soundings. Re-ran for all 20 facilities: several point estimates changed substantially (ShriSingajiMalwa +320%, Chhabra −93%, Tirora −90%), confirming the seasonal-imbalance bug was real and not isolated to ShriSingajiMalwa. Re-ran the Climate TRACE comparison at the same N=17: **calibration dropped from 53% (9/17) to 35.3% (6/17)** — a materially worse-looking number, reported honestly rather than hidden. This is not evidence the fix was wrong: several facilities' month-restricted background sample sizes dropped sharply (e.g. Tirora 524→127 soundings, RGundem 80→61), trading a systematic seasonal bias for increased sampling noise — a new caveat, not a regression in methodology. Old (pre-fix) estimates preserved as `data/emission_estimates_v1_no_month_stratification.json` / `data/climate_trace_comparison_v1_no_month_stratification.json`. (b) **Per-plant process locking** in `process_plant.py`: an exclusive, non-blocking `fcntl.flock` on `data/{NAME}.lock`, acquired before the OCO-3 scan begins, so a second process for the same plant fails fast instead of silently racing the first (the actual 2026-08-13 bug — two independent scan loops, not just an unprotected checkpoint write). Verified in isolation: a second lock attempt on the same file correctly raises `OSError`. (c) **Added `requirements.txt`**, pinned to the working conda env (`/opt/miniconda3/envs/co2`, Python 3.11.15). Writing it surfaced an unrelated, previously-undocumented finding: scripts have also been run under a second, separate Python (the macOS python.org 3.14 framework build on `/usr/local/bin/python`) missing `earthaccess`/`xarray`/`geemap` — anything importing those (`process_plant.py`, OCO-3 scripts) would fail under it, though this session's numpy/torch/pandas/ee-only scripts happened to work under either, which is how it went unnoticed. Flagged in `requirements.txt` itself.

**12.16 Resolved TalwandiSabo's negative CO2 enhancement.** Committing TalwandiSabo's `plant_results.json` row (§12.15's facility-set commit) surfaced the same anomaly shape as §12.6–§12.7: a negative near-minus-background enhancement (−1.277 ppm). Wrote `diagnose_talwandisabo.py`, reusing `diagnose_negative_enhancement.py`'s significance check and `diagnose_shrisingajimalwa.py`'s seasonal same-month comparison directly (no reimplementation). **Stage 1: the negative sign is statistically significant** (z=−7.34, not consistent with zero) — the same situation ShriSingajiMalwa was in, not Koradi/Tamnar's noise-only case. **Stage 2: all 37 near-plant soundings are from February, while the background ring blends February/May/December** (mean bg month 5.7 vs. near month 2.0). Restricting the comparison to the one shared month (February) **flips the sign positive**: +0.152 ppm (z=1.26, not itself significant but correctly signed) — the same seasonal-sampling-imbalance artifact as ShriSingajiMalwa, not a real negative signal or pipeline bug. Full stats in `data/talwandisabo_negative_enhancement_diagnosis.json`. One minor, non-blocking issue noted while running this: `bg_definition_sensitivity()` (from `diagnose_talcher.py`) returns `NaN` for TalwandiSabo's IME-proxy-range percentage (a `ime_proxies[0]` divide-by-zero edge case) — doesn't affect this diagnosis's conclusion, since that metric is supplementary to the significance/seasonal analysis, but worth a defensive fix if this function is reused again.

**12.14 First test suite (`tests/`, closing item 9's remaining scope).** Added 22 stdlib-`unittest` tests (no new dependency) covering the deterministic building blocks of the pipeline most likely to silently produce a wrong number, per this session's own bug history: `test_physics_ime.py` (IME math, the new month-stratification logic), `test_build_3channel.py` (NO2/SO2/VIIRS tile pairing, gap-filling, SO2/VIIRS negative-value clamping), and `test_lofo_track_a.py` (`facility_fold_indices()`, directly guarding against a regression of Week 11's tile-level-leakage bug). Required two small, behavior-preserving refactors to make modules safely importable: `build_3channel.py` gained an `if __name__ == "__main__":` guard (it previously ran its full pairing pipeline at import time), and `lofo_track_a.py`'s inline fold-splitting logic was extracted into a top-level `facility_fold_indices()` function. All 22 tests pass (`python -m unittest discover -s tests -v`, ~0.1s, no network/GPU required). Deliberately excludes integration paths (Earth Engine, OCO-3 downloads, end-to-end model training) — those are exercised by running the actual pipeline scripts, not unit tests. No CI configured to run these automatically.

**12.15 Found a real, independent ground-truth source and built the actual Q-correcting model (§11 item 3, the key remaining blocker).** The A1→A5 ladder needed an independent, non-Climate-TRACE emissions source; §12.11 concluded none existed and worked around it with a self-consistency-only fusion attempt. Research this session found one: the **Central Electricity Authority (CEA)'s CO2 Baseline Database** — a Government of India, Ministry of Power publication, free and login-free at `cea.nic.in`. Critically, it is fundamentally different in kind from Climate TRACE: **bottom-up**, computed from each plant's own reported fuel consumption × gross calorific value × IPCC/national emission factor (the same accounting basis used for real CDM carbon-credit determinations), not satellite/thermal-proxy inference. This makes it usable as a genuine label under RESEARCH_PLAN.md §7's restriction, which is explicitly scoped to Climate TRACE.

Wrote `pull_cea_ground_truth.py`, downloading FY2020-21 data (Version 17.0, the closest single-fiscal-year match to Track B's 2020-calendar-year OCO-3 estimates — India's fiscal year is April–March, so FY2020-21 overlaps 9 of the 12 sounding-collection months). **All 30 candidate facilities matched by name** (20 committed + 10 from the paused expansion batch), several so closely (e.g. CEA's "VINDH_CHAL STPS" vs. this project's own "VINDH_CHAL_STPS" internal alias) that the original facility set may well have been sourced from this same database originally.

Wrote `q_correction_model.py` — the actual correction model, evaluated by leave-one-out CV at N=17 (facilities with both a Track B estimate and a CEA-matched, activity-signal-available row):
- **Stage 1, baseline accuracy against real ground truth:** MAE of 1.01 in log-ratio space (log(ours/CEA), the correct scale for multiplicative error) — roughly a 2.75× typical factor of error — with a systematic underestimate bias (mean log-ratio −0.53) and a **47.1% bracketing rate (8/17)**, meaningfully different from (and independently informative alongside) the Climate-TRACE-based 35.3%.
- **Stage 2, single-feature LOO correction:** `bg_std_ppm` (background XCO2 noise) is the strongest predictor of log-ratio error (r=+0.58) — physically sensible, since a noisier background zone makes the near-minus-background signal less reliable. Correcting each facility's estimate by this feature's LOO-predicted bias **reduced MAE from 1.012 to 0.902**, a genuine ~11% improvement validated honestly under leave-one-out CV, not a fit-and-declare result.

**This is the first time this project has built a Q-correcting model against real, non-proxy ground truth** — the actual milestone RESEARCH_PLAN.md §9 called for, previously blocked all session on the absence of exactly this kind of source. Still explicitly indicative, not production-validated: N=17, single-feature linear correction only (per the same small-N discipline as `reliability_model.py`), one fiscal year, one CEA version. Full results in `data/cea_ground_truth_2020_21.json` and `data/q_correction_model_results.json`.
...
- Varon, D. J., et al., 2018. (Source of the IME method and the α=0.5 effective-wind-speed scaling factor used in `physics_ime.py`; originally developed for CH4 plume quantification.)
