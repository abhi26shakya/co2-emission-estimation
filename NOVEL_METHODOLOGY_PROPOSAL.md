...
- **Per-overpass wind-to-sounding date matching** for the spatial consistency test — the most likely fix for the negative §11 result, testable without new data (the `day` field already exists per sounding).
- **Phase 3** (Grad-CAM spatial fusion) and beyond, per §5's table — deprioritized further given §11's result; building more on top of an unvalidated spatial claim isn't productive until the day-matching fix is tried.
- **Paper framing decision**: given §11, the strongest, most defensible headline claim for a paper right now is the CEA ground-truth-validated Q-correction (already solid, §1), with the plume/hotspot work presented as an honest, partially-negative methodological exploration — itself a valid contribution (a rigorous permutation-test methodology for validating satellite-derived spatial claims), just not the positive spatial-accuracy result originally hoped for.
